...
def preprocess_data(data, _type):
    ...
    # filter out.tex rbocpdms on "best" (uni or multi)
    if _type == "best":
        warning(
            "\nWarning: Filtering out.tex RBOCPDMS due to insufficient results.\n"
        )
        methods = [m for m in methods if not m == "rbocpdms"]

    # filter out.tex methods that have no results on any dataset
    ...
    if methods_no_result:
        print(
            "\nWarning: Filtering out.tex %r due to no results on any series\n"
            % methods_no_result,
            file=sys.stderr,
        )
